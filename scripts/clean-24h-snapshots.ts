...
async function clean24h() {
  console.log('🔥 Cleaning first 24h of snapshots...\n');
  ...
  for (const model of models || []) {
    ...
    const cutoffTime = firstTime + (24 * 60 * 60 * 1000);
    ...
    if (toDelete.length > 0) {
      console.log(`${model.name}: Deleting ${toDelete.length} snapshots from first 24h`);
